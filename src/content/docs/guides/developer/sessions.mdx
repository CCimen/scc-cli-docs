---
title: Session Management
description: Managing agent sessions in SCC
sidebar:
  order: 3
---

import { Aside } from '@astrojs/starlight/components';
import DrawioDiagram from '../../../../components/DrawioDiagram.astro';

Sessions are the core unit of work in SCC. Each session is an agent environment (Claude Code or Codex) running in a container, tied to a specific workspace and provider.

## Session Lifecycle

<DrawioDiagram src="/diagrams/session-lifecycle-detailed2.svg" alt="Session Lifecycle" />

## Starting Sessions

### Smart Start (Recommended)

```bash
cd ~/project
scc
```

This auto-detects your workspace and shows Quick Resume if sessions exist.

### Explicit Start

```bash
scc start ~/project
```

### Start Options

| Option | Description |
|--------|-------------|
| `--resume` | Resume most recent session |
| `--select` | Interactive session picker |
| `--fresh` | Force new container |
| `--team TEAM` | Use specific team profile (session-only) |
| `--session NAME` | Name this session |
| `--dry-run` | Preview config without launching |

## Listing Sessions

```bash
scc sessions
```

By default, this shows sessions for your **active team**. Use `--all` to see
sessions across teams, or `--team <name>` to filter explicitly.

```
 Sessions

  › feature-auth      draken · project-a · main · 2h ago
    bugfix-login      draken · project-a · fix/login · 1d ago
    main              draken · project-b · main · 5h ago
```

```bash
# Show sessions for all teams
scc sessions --all

# Show sessions for a specific team
scc sessions --team draken
```

## Resuming Sessions

### Resume Most Recent (Team-Scoped)

```bash
scc start --resume
```

### Pick from List

```bash
scc start --select
```

### Resume Specific Session

```bash
scc start --session feature-auth
```

<Aside type="note">
  Sessions can only be resumed if the container still exists. Containers are preserved until explicitly stopped or pruned.
</Aside>

## Stopping Sessions

### Stop Specific Container

```bash
scc stop <container-name>
```

Find the container name using `scc list` or `docker ps` / `podman ps`.

### Stop All SCC Containers

```bash
scc stop
```

This stops all running SCC containers at once.

<Aside type="note">
  Use `scc list` to see running containers and their names before stopping specific ones.
</Aside>

## Session Naming

Name your sessions for easier identification:

```bash
scc start --session "feature-payments" ~/project
```

When you have multiple features in progress, named sessions help:

```
Recent sessions:
  1. feature-payments (2h ago)
  2. bugfix-auth (1d ago)
  3. spike-new-api (3d ago)
```

## Dashboard & Quick Resume

The dashboard appears when you run `scc` and shows a tabbed interface with Status, Containers, Sessions, and Worktrees:

```
 Status   Containers   Sessions   Worktrees

  › New session
    Resume last                draken · main · 2h ago
    Team: draken
    Containers: 2/3 running
    Settings

↑↓ navigate  ·  Enter open  ·  Tab switch tab  ·  n new  ·  q quit
```

<Aside type="tip">
  Press **Tab** to switch between tabs. The Sessions tab shows all recent sessions for your team.
</Aside>

## Session Context

SCC tracks context for each session, including:
- Last working directory
- Active files
- Session duration

Clear stale context:

```bash
scc context clear
```

## Session Timeouts

Sessions can be configured to timeout after inactivity:

```yaml title="Team profile or .scc.yaml"
session:
  timeout_hours: 8
  auto_resume: true
```

| Setting | Default | Description |
|---------|---------|-------------|
| `timeout_hours` | 24 | Hours before session expires |
| `auto_resume` | true | Advisory only in v1 (not enforced) |

<Aside type="note">
  **Note:** Timeout configuration is available but enforcement is not yet implemented. Sessions will not automatically expire based on these settings, and auto-resume is advisory.
</Aside>

## Container Cleanup

### View Running Containers

```bash
scc list

# or
scc container
```

### Prune Stopped Containers

```bash
scc prune
```

<Aside type="caution">
  Pruned containers cannot be resumed. Only prune sessions you're done with.
</Aside>

### Prune with Dry Run

```bash
scc prune --dry-run
```

Shows what would be removed without actually removing.

## Session History Cleanup

Over time, session history can accumulate. Use `scc sessions prune` to clean up:

```bash
# Preview what would be removed
scc sessions prune --dry-run

# Prune sessions older than 30 days (keeps 20 most recent per team)
scc sessions prune

# Keep only last 10 sessions per team
scc sessions prune --keep 10

# Prune sessions older than 7 days
scc sessions prune --older-than 7d
```

<Aside type="tip">
  You can also access session cleanup from the TUI: press `s` from the dashboard to open Settings & Maintenance.
</Aside>

## Troubleshooting Sessions

### Session Won't Resume

**Cause:** Container was pruned or crashed.

**Solution:** Start fresh:
```bash
scc start --fresh
```

### Quick Resume Not Showing

**Cause:** Running from a different path than the original session, or viewing a different team.

**Solution:** Navigate to the exact workspace root and (if needed) show all teams:
```bash
cd ~/project  # Not ~/project/src
scc

# In Quick Resume, press 'a' to show all teams
# Or list sessions across teams
scc sessions --all
```

### Session Stuck

**Cause:** Container in bad state.

**Solution:** Stop container and restart:
```bash
# Find the container name
scc list

# or
scc container

# Stop the specific container
scc stop <container-name>

# Start fresh
scc start --fresh
```
