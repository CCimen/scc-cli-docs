...
<Aside type="note">
  Team config files support: `schema_version` (semver string, currently `1.0.0`), `enabled_plugins`, `disabled_plugins`, and `marketplaces`. Federated teams skip `defaults.allowed_plugins` filtering and are trusted via trust grants instead.
</Aside>
