...
For fine-grained control, configure individual command permissions:
...
```json
{
  "security": {
    "safety_net": {
      ...
      "block_branch_delete": true,
      "block_clean": true,
      "block_stash_drop": true,
      "block_stash_clear": true,
      "block_checkout_file": false,
      "block_restore_file": false
    }
  }
}
```
...
|--------|---------|-------------|
| `block_force_push` | `true` | Block `git push --force` and `-f` |
| `block_reset_hard` | `true` | Block `git reset --hard` |
| `block_branch_delete` | `true` | Block `git branch -D` |
| `block_clean` | `true` | Block `git clean -fd` |
| `block_stash_drop` | `true` | Block `git stash drop` |
| `block_stash_clear` | `true` | Block `git stash clear` |
| `block_checkout_file` | `true` | Block `git checkout <file>` |
| `block_restore_file` | `true` | Block `git restore <file>` |

<Aside type="tip">
  Use granular controls when you need to allow specific destructive commands while blocking others. For example, allow file restoration but block force push.
</Aside>
