...
<Aside type="danger">
  These commands can cause irreversible data loss. SCC's built-in safety engine provides cross-provider baseline protection; the plugin adds Claude Code agent-native hooks on top.
</Aside>
...
<Aside type="tip">
  Always enable `scc-safety-net` for Claude Code teams unless you have a specific reason not to. If your organization also uses Codex, keep the plugin enabled where it applies and rely on SCC's built-in safety engine for the cross-provider baseline.
</Aside>

## How It Works

The safety net plugin uses agent-native hooks to intercept bash commands before execution. It supports **Claude Code** via Claude hooks. SCC also includes a **built-in safety engine** with shell wrappers that provides defense-in-depth protection for all providers, independent of the plugin:
