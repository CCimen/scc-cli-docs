...
The safety net plugin uses agent-native hooks (Claude Code hooks, Codex rules) to intercept bash commands before execution. Note that SCC also includes a **built-in safety engine** with shell wrappers that provides defense-in-depth protection independent of the plugin:
