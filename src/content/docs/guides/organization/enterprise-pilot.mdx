...
| Dev environment commands | Use named `dev_environment.commands` only as an explainable contract until bridge execution is implemented |
...
- `dev_environment.commands` appear in explain output with fixed `argv` lists
  and are not executed by SCC yet;
...
- Which named dev-environment commands were accepted, denied, or ignored?
...
- execution of named `dev_environment.commands`;
