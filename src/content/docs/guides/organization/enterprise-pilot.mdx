...
- `dev_environment.commands`, `dev_environment.logs`, and
  `dev_environment.health_checks` appear in explain/status output with fixed
  `argv` lists;
- `scc dev status . --json` reports the effective bridge contract without
  executing a host process;
- `scc dev run test . --json`, `scc dev logs api . --json`, and
  `scc dev health api . --json` write pre-execution audit events, return
  bounded stdout/stderr tails, and do not put stdout/stderr content in durable
  audit metadata;
...
- For devcontainer or Compose projects, did the "Dev Environment Bridge" doctor
  check list the detected files and state that SCC does not mount the Docker
  socket into agent containers or attach them to arbitrary project networks?
