...
<Aside type="tip" title="Use the Wizard">
For a faster setup experience, use the SCC Wizard plugin: `/scc-wizard:org`

The wizard generates a working config in ~60 seconds with just 2 inputs. See [Wizard Guides](/guides/wizard/overview) for details.
</Aside>
...
<Aside type="caution">
  `schema_version` must be `1.0.0`. Other versions are rejected.
</Aside>
...
4. **Set organization defaults**
   ...
   > **Note:** `session.auto_resume` is advisory only in v1. It is accepted but not enforced at runtime.
