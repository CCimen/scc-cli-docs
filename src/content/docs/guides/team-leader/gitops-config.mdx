...
<Aside type="note">
  Federated team configs only support `enabled_plugins`, `disabled_plugins`, and `marketplaces`.
  MCP servers and session settings must be configured at the organization level.
</Aside>
