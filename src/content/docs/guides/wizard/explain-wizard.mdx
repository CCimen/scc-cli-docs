...
| `network_policy` | Inherited policy with stricter-only team/project overrides |
| `cache_ttl_hours` | Org defaults only |

<Aside type="caution">
Team and project `network_policy` values can only be **as restrictive or more restrictive** than the inherited policy. Looser values are ignored and surfaced by `scc config explain --field network` under "Ignored Policy Changes". `session.auto_resume` is advisory only in v1.
</Aside>
