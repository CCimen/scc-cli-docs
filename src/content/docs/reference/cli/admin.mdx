...
| `--compliance` | Add the `enterprise_audit_v1` evidence index and section checksums |
...
- Launch and safety audit summaries
- Runtime, provider, policy, work-context, and governed-artifact diagnostics

`--compliance` keeps the same support-bundle zip and JSON manifest format. It
adds a deterministic evidence index with SHA-256 checksums for the emitted
redacted manifest sections, plus explicit non-goals. It is audit evidence for
reviewers, not a SOC 2/ISO certification, SSO/SCIM implementation, SBOM
attestation, dashboard, project registry, new provider, or Docker socket access.
...
<Tabs>
  ...
  # Include enterprise audit evidence index
  scc support bundle --compliance

  # Pretty-print JSON
  scc support bundle --compliance --pretty
  ```
  </TabItem>
</Tabs>
...
| `--containers` | Low | Prune stopped containers |
