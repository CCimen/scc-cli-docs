...
Shows the effective configuration with source tracking, enforcement status, ignored policy changes, and warnings:
...
It also includes `data.ignored_policy_changes` for team or project policy values that were rejected because they would loosen inherited controls.

### scc config validate

Validates project config (`.scc.yaml`) against team/org policy gates and reports blocked additions, denied additions, and ignored project policy values.
...
Configure agent status line integration.
