...
The host-visible path passed to the container runtime for the workspace bind mount. It normally matches the workspace path. When SCC runs inside a devcontainer with the host Docker socket, `SCC_WORKSPACE_PATH_MAP` can translate a path such as `/workspaces/app` to the host path the Docker daemon can mount.
...
### Safety Net
The `scc-safety-net` plugin that provides additional protection via agent-native hooks where supported. Complementary to the built-in safety engine.
