...
to inspect or execute accepted commands. `scc dev run <name>` writes a
pre-execution audit event before launching the host process, rejects unknown
command names, rejects `working_directory` values that escape the workspace
after symlink resolution, and returns bounded stdout/stderr tails.
