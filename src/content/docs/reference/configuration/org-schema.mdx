...
<Aside type="note">
  Profile-level `network_policy` overrides the org default. It can only be **more restrictive** than the org default (e.g., `open` → `web-egress-enforced` is allowed, but `locked-down-web` → `open` is not).
</Aside>
