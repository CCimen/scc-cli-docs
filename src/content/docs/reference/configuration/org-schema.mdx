...
| `network_policy` | string | Network policy override (`unrestricted`, `corp-proxy-only`, `isolated`) |
...
<Aside type="note">
  Profile-level `network_policy` overrides the org default. It can only be **more restrictive** than the org default (e.g., `unrestricted` → `corp-proxy-only` is allowed, but `isolated` → `unrestricted` is not).
</Aside>
