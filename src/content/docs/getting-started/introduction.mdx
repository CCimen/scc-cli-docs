---
title: What is SCC?
description: Introduction to SCC, a governed runtime for Claude Code and Codex with container sandboxes, network controls, team-managed profiles, and safer enterprise rollout
sidebar:
  order: 1
---

import { Card, CardGrid, Aside } from '@astrojs/starlight/components';
import DrawioDiagram from '../../../components/DrawioDiagram.astro';
import FAQSchema from '../../../components/FAQSchema.astro';

**SCC (Sandboxed Coding CLI)** is a provider-neutral governed runtime for AI coding agents. It runs [Claude Code](https://docs.anthropic.com/en/docs/claude-code) and [Codex](https://openai.com/index/introducing-codex/) inside container sandboxes (Docker Engine, OrbStack, Colima, or Docker Desktop) with organization-managed team profiles, a built-in safety engine, and git worktree support.

<Aside type="tip">
  SCC isolates agent execution in containers, enforces web egress through network topology, runs a fail-closed safety engine, and distributes team configuration through a central org config — developers get standardized setups without manual configuration, regardless of which agent they use.
</Aside>

<FAQSchema
  faqs={[
    {
      question: 'What problem does SCC solve for teams using AI coding agents?',
      answer:
        'SCC gives teams one governed runtime for AI coding agents. It standardizes setup, contains filesystem access in a container, adds network controls, and applies team policy without asking every developer to assemble the same toolchain by hand.',
    },
    {
      question: 'Can SCC run both Claude Code and Codex?',
      answer:
        'Yes. SCC supports Claude Code and Codex today. Organizations can allow one provider or both while keeping the same governance model, onboarding flow, and safety baseline.',
    },
    {
      question: 'Is a container alone enough for safe AI coding?',
      answer:
        'No. Containers help with filesystem and process isolation, but they do not automatically limit outbound network reach. SCC adds network policies and topology-enforced web egress to reduce that risk.',
    },
  ]}
/>

## Read This First

You do not need to read every page to start using SCC.

- If you want to **get running fast**, read [Quick Start](/getting-started/quick-start/), then [Core Concepts](/getting-started/core-concepts/), then [Daily Workflow](/guides/developer/daily-workflow/).
- If you are **evaluating SCC for a team or organization**, read this page, then [Security Model](/architecture/security-model/), then [Architecture Overview](/architecture/overview/).
- If your main concern is **rolling out AI coding safely across teams**, read [AI Coding Agent Governance](/use-cases/ai-coding-agent-governance/) after this page.
- If your main concern is **what the agent can reach on your network**, read [Network Controls for AI Coding Agents](/use-cases/network-controls-for-ai-coding-agents/) after this page.
- If you are **setting policy**, continue from there into the organization and team-leader guides.

## The Problem SCC Solves

When teams use AI coding agents, several challenges emerge:

<CardGrid>
  <Card title="Security Risk" icon="warning">
    AI agents can execute arbitrary commands on your host machine, including destructive ones like `rm -rf` or `git push --force`.
  </Card>
  <Card title="Configuration Drift" icon="setting">
    Each developer sets up their environment differently, leading to inconsistent behavior and hard-to-reproduce issues.
  </Card>
  <Card title="Plugin Chaos" icon="puzzle">
    Without governance, developers install random plugins that may conflict or introduce security vulnerabilities.
  </Card>
  <Card title="Provider Lock-in" icon="github">
    Teams want to evaluate or switch between AI coding agents without rebuilding their governance, config, and plugin infrastructure.
  </Card>
</CardGrid>

## How SCC Helps

### Provider-Neutral Runtime

SCC supports multiple AI coding agents through a provider-neutral architecture. Claude Code and Codex are first-class providers today. The architecture is extensible — adding a new provider means adding one adapter, not changing the core.

```bash
scc provider show          # See current preference
scc provider set codex     # Switch default provider
scc start --provider claude ~/project  # Override for one session
```

Your org config, team plugins, safety policies, and governance carry over regardless of which provider you use. Teams can standardize on one provider, allow both, or mix them by workflow. That makes SCC easier to adopt in organizations where one team prefers Claude for exploratory work and another prefers Codex for backend-heavy tasks.

### Container Isolation

Every agent session runs inside an OCI container. The agent can only access files you explicitly mount, and it cannot affect your host system directly.

<DrawioDiagram src="/diagrams/docker-sandbox-isolation.svg" alt="Container Sandbox Isolation" />

### Containers Are Necessary, Not Sufficient

Container isolation protects the host filesystem boundary, but it does not automatically solve network risk. A normal container can still reach the internet or internal services that your runtime can route to.

SCC separates those concerns clearly:

- **Container isolation** limits filesystem and process scope
- **`open`** keeps normal outbound network access
- **`web-egress-enforced`** routes HTTP/HTTPS through a Squid proxy sidecar on an internal-only network
- **`locked-down-web`** disables network access entirely with `--network=none`

That distinction matters when teams worry about agents reaching internal APIs, metadata endpoints, or services outside the current workspace.

### Built-in Safety Engine

SCC includes a fail-closed safety engine that runs inside every container:

- **Git safety rules** — blocks `push --force`, `reset --hard`, `branch -D`, `clean -fd`, and other destructive git commands
- **Network tool interception** — wraps curl, wget, ssh, scp, sftp, and rsync with policy checks
- **Shell tokenizer** — parses compound commands to catch destructive operations hidden in pipes or subshells
- **Fail-closed** — if safety policy cannot be loaded, all guarded commands are blocked by default

The optional **scc-safety-net** plugin provides additional agent-native hook coverage where supported; SCC's built-in safety engine is the provider-neutral enforcement baseline.

### Centralized Configuration

Organization admins define a single JSON config that specifies:
- Which plugins are allowed/blocked
- Which MCP servers teams can use
- Security policies (network posture, safety-net action)
- Team-specific profiles

Developers just run `scc setup` once and they're ready.

### Topology-Enforced Web Egress

When `network_policy` is set to `web-egress-enforced`, SCC creates an internal-only Docker network. The agent container can only reach the internet through a Squid proxy sidecar that enforces an ACL. This is real network topology enforcement — even if the agent ignores proxy environment variables, it physically cannot bypass the proxy. The compiled ACL also denies loopback, private CIDRs, link-local ranges, and cloud metadata endpoints before any allow rules are applied.

### Git Worktree Integration

SCC uses git worktrees to create isolated branches for each task:

```bash
scc worktree create ~/project feature-auth
# Creates isolated branch scc/feature-auth
# Auto-starts an agent session in the worktree
```

## Who Uses SCC?

| Role | What SCC Provides |
|------|-------------------|
| **Developers** | Instant setup, protected main branch, isolated experiments, provider choice |
| **Team Leaders** | Approved plugin sets, consistent configs across the team |
| **Org Admins** | Security policies, blocked plugins, audit trails, provider governance |

## Requirements

- **Python 3.10+**
- **Container runtime** — Docker Engine 20.10+, OrbStack, Colima, or Docker Desktop (Podman is not validated in v1)
- **Git 2.30+**
- **uv** (recommended for installation)

## Next Steps

<CardGrid>
  <Card title="Quick Start" icon="rocket">
    Ready to try it? [Install and run in 30 seconds →](/getting-started/quick-start/)
  </Card>
  <Card title="Core Concepts" icon="open-book">
    Want to understand more? [Learn the key concepts →](/getting-started/core-concepts/)
  </Card>
  <Card title="Security Model" icon="shield">
    Need the network and isolation details? [Review the security layers →](/architecture/security-model/)
  </Card>
  <Card title="Why Sandbox AI Coding Agents" icon="warning">
    Evaluating the risk model? [See where containers help and where they do not →](/use-cases/why-sandbox-ai-coding-agents/)
  </Card>
</CardGrid>
