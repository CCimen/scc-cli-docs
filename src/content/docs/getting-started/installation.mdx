...
SCC requires a Docker-compatible container runtime. Validated v1 runtimes are:
...
| [Podman](https://podman.io/) | Linux, macOS | Not validated in v1 |
...
<Aside type="note">
  Installing a container runtime gives SCC a sandbox boundary, but it does not automatically restrict outbound traffic from agent sessions. Network control is configured separately through SCC's `network_policy` settings.
</Aside>
