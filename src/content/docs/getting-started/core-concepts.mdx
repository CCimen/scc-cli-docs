...
**Provider preference resolution:** `--provider` CLI flag > workspace last-used > global preference > prompt (when multiple providers are available).

SCC remembers the last-used provider per workspace in `.scc/config.local.json` (gitignored), so repeated launches default to what you used last time.
...
The optional **scc-safety-net** plugin provides additional coverage via agent-native hooks.
