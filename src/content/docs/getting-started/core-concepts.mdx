...
3. If global preference is `ask` → always prompt (skips workspace memory)
4. Workspace last-used (from `.scc/config.local.json`, gitignored)
5. Global preference (`claude` or `codex`)
6. Interactive prompt (when multiple providers are available)

SCC remembers the last-used provider per workspace, so repeated launches default to what you used last time — unless you set `scc provider set ask`, which always prompts.
...
The most important official plugin is **scc-safety-net**, which provides agent-native hook-based protection in addition to SCC's built-in safety engine.
