...
3. **Resolves provider** — uses `--provider` flag, workspace last-used, global preference, or prompts if multiple providers are available
4. **Checks readiness** — verifies provider image exists and auth is valid; auto-builds image or triggers browser sign-in if needed
...
| "Provider image missing" | SCC auto-builds on first start. Manual: `docker build -t scc-agent-claude images/scc-agent-claude/` |
