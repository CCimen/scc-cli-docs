...
**Fix:** Install plugins through your organization config or your agent (Claude Code / Codex).
...
**Symptom:** Plugin installed but not available in the agent session.
...
Claude Code stores plugin information at (Codex uses `.codex/config.toml` instead):
...
<Aside type="note">
  SCC doesn't download or cache plugins directly — the agent (Claude Code or Codex) handles plugin installation. SCC only reads the registry to audit and validate against governance.
</Aside>
