...
<Aside type="caution">
  Container isolation is only one layer. A normal container can still reach network destinations your runtime allows. SCC treats filesystem/process isolation and outbound egress as separate controls so teams can decide whether a provider should run with open web access, topology-enforced web access, or no network at all.
</Aside>
...
SCC uses a portable OCI runtime path through the Docker CLI. Validated v1 runtimes are:
...
Each provider gets a persistent named volume for credential and data persistence. Container names are deterministic, derived from the runtime mount source and provider ID — running Claude and Codex in the same workspace produces separate, identity-isolated containers. In devcontainers that use host Docker socket path mapping, the runtime mount source is the host-visible path, so host and devcontainer launches converge on the same Docker-daemon identity.
