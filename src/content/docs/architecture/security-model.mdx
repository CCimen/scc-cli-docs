...
- Unlisted destinations, including direct IP-literal attempts, hit the terminal deny-all rule
...
SCC does not join arbitrary project, devcontainer, or Docker Compose service
networks in v1. Under `web-egress-enforced`, the agent container must stay on
SCC's internal network so the proxy sidecar remains the only egress bridge.
...
Patterns in `security.blocked_*` are organization-level blocks:
- Cannot be overridden directly by teams
- Cannot be overridden directly by projects
- Cannot be overridden by local/user overrides
- Can be overridden only by audited, time-bounded policy exceptions
