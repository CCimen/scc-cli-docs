...
Execution is fail-closed around audit: SCC writes a pre-execution audit event
before starting the host process. If that audit write fails, the command does
...
Command output is returned as bounded stdout/stderr tails and is not written to
