...
SCC runs agents inside OCI containers through the Docker CLI using Docker Engine, OrbStack, Colima, or Docker Desktop:
...
The optional `scc-safety-net` plugin adds extra agent-native coverage where supported. SCC's built-in safety engine remains the provider-neutral baseline.
