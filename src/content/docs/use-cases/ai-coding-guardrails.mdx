...
<FAQSchema faqs={[
  ...
  { question: "How does SCC implement guardrails for AI coding agents?", answer: "SCC implements guardrails through: container isolation (filesystem boundaries), a built-in fail-closed safety engine (blocks destructive git commands and network tools), plugin governance (org-approved plugins only), topology-enforced network egress, and team profiles (consistent configuration)." },
...
### Layer 2: Built-in Safety Engine
...
The optional **scc-safety-net** plugin adds agent-native hook coverage on top where supported.
...
<Aside type="note">
  `web-egress-enforced` uses real network topology enforcement: the agent container runs on an internal-only Docker network with a Squid proxy sidecar as the sole gateway. `locked-down-web` uses `--network=none`. Enforcement is HTTP/HTTPS over IPv4 in v1; raw TCP/UDP is not filtered.
</Aside>
